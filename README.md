...
> ⚠️ **이 프로젝트는 더 이상 사용되지 않습니다 (Deprecated)**
>
> **2025년 11월 26일부로 이 프로젝트는 정상 동작하지 않습니다.**
>
> - Notion Report 아이템의 `isToday`, `isTomorrow` 속성이 삭제되어 기존 로직이 동작하지 않습니다.
> - 이 프로젝트는 [watchtek-daily-report-web](https://github.com/uhjee/watchtek-daily-report-web)으로 완전히 이관되었습니다.
>
> 새로운 프로젝트를 사용해 주세요.
...
`src/config/members.ts` 파일을 생성하고 팀원 정보를 입력합니다.
...
- **TypeScript** (v5.7.2) - 타입 안정성이 보장되는 JavaScript
...
`executable/` 폴더에 `.exe` 파일이 생성됩니다. 이 파일과 함께 `.env` 파일을 같은 디렉토리에 배치하여 사용하세요.

### 시스템 서비스 등록 (Windows)

Windows 작업 스케줄러를 사용하여 시스템 시작 시 자동 실행:

1. Windows 검색에서 "작업 스케줄러" 실행
2. "기본 작업 만들기" 클릭
3. 이름: "Watchtek 보고서 자동화"
4. 트리거: "컴퓨터를 시작할 때"
5. 작업: "프로그램 시작"
6. 프로그램: `watchtek-daily-report.exe` 경로 선택
7. 완료
...
- **[CLAUDE.md](./CLAUDE.md)** - AI 개발 가이드 (Claude Code용)
